...
function initWhatsApp(key) {
  ...
  try {
    ...
    // Windows local dev mein Edge use karo
...
function smartDelay() { return Math.floor(4000 + Math.random() * 11000); }
...
function handleSupabaseError(res, context, error, fallback = 'Database request failed') {
  ...
  return res.status(500).json({ error: fallback, code: error?.code || 'SUPABASE_ERROR' });
}
...
app.post('/api/auth/signup', async (req, res) => {
  try {
    ...
    if (existingError) return handleSupabaseError(res, 'signup check existing user', existingError, 'Could not check existing user');
    if (existing) return res.status(409).json({ error: 'Email is already registered' });

    const passwordHash = await bcrypt.hash(password, 12);
    const { data: user, error } = await supabase
      .from('app_users').insert({ email, password_hash: passwordHash, name })
      .select('id, email, name').single();

    if (error) return handleSupabaseError(res, 'signup insert app_users', error, 'Signup failed');
...
app.post('/api/auth/login', async (req, res) => {
  try {
    ...
    const { data: user, error } = await supabase
      .from('app_users').select('id, email, name, password_hash').eq('email', email).maybeSingle();

    if (error) return handleSupabaseError(res, 'login select app_users', error, 'Login failed');
...
app.get('/api/contacts/stats', requireAuth, async (req, res) => {
  try {
    ...
    const { count: total, error: totalError } = await supabase
      .from('contacts').select('*', { count: 'exact', head: true }).eq('user_id', userId);

    if (totalError) return handleSupabaseError(res, 'contacts stats total', totalError, 'Stats fetch failed');

    const typeCounts = await Promise.all(types.map(type =>
      supabase.from('contacts').select('*', { count: 'exact', head: true }).eq('user_id', userId).eq('type', type)
    ));

    const unitCounts = await Promise.all(units.map(unit =>
      supabase.from('contacts').select('*', { count: 'exact', head: true }).eq('user_id', userId).eq('unit', unit)
    ));

    const failedType = typeCounts.find(r => r.error);
    if (failedType) return handleSupabaseError(res, 'contacts stats type count', failedType.error, 'Stats fetch failed');

    const failedUnit = unitCounts.find(r => r.error);
    if (failedUnit) return handleSupabaseError(res, 'contacts stats unit count', failedUnit.error, 'Stats fetch failed');

    const stats = { all: total || 0 };
    types.forEach((type, i) => { stats[type] = typeCounts[i].count || 0; });
    units.forEach((unit, i) => { stats[unit] = unitCounts[i].count || 0; });
...
app.get('/api/contacts', requireAuth, async (req, res) => {
  try {
    let query = supabase.from('contacts').select('*').eq('user_id', req.user.id).order('name');
    if (req.query.type && req.query.type !== 'all') query = query.eq('type', req.query.type);
    if (req.query.unit && req.query.unit !== 'all') query = query.eq('unit', req.query.unit);

    const { data, error } = await query.limit(10000);
    if (error) return handleSupabaseError(res, 'contacts list', error, 'Contacts fetch failed');
...
app.post('/api/contacts/bulk-upsert', requireAuth, async (req, res) => {
  try {
    ...
    const userRows = rows.map(row => ({ ...row, user_id: req.user.id }));
    const { data, error } = await supabase.from('contacts')
      .upsert(userRows, { onConflict: 'user_id,mobile' }).select('id');

    if (error) return handleSupabaseError(res, 'contacts bulk upsert', error, 'Contacts save failed');
...
app.patch('/api/contacts/:id', requireAuth, async (req, res) => {
  try {
    ...
    if (!Object.keys(updates).length) return res.status(400).json({ error: 'No allowed contact fields provided' });

    const { data, error } = await supabase.from('contacts')
      .update(updates).eq('id', req.params.id).eq('user_id', req.user.id).select('*').maybeSingle();

    if (error) return handleSupabaseError(res, 'contacts update', error, 'Contact update failed');
...
app.delete('/api/contacts/:id', requireAuth, async (req, res) => {
  try {
    const { data, error } = await supabase.from('contacts')
      .delete().eq('id', req.params.id).eq('user_id', req.user.id).select('id').maybeSingle();

    if (error) return handleSupabaseError(res, 'contacts delete', error, 'Contact delete failed');
    if (!data) return res.status(404).json({ error: 'Contact not found' });
...
app.get('/api/wa/status/all', requireAuth, (req, res) => {
  ...
  for (const key of ['ugi', 'unit1', 'unit2']) {
    ...
    result[key] = {
      status: wa.status,
      qr: wa.qr ? `https://api.qrserver.com/v1/create-qr-code/?size=250x250&data=${encodeURIComponent(wa.qr)}` : null
    };
  }
  res.json(result);
});
...
app.post('/api/blast/start', requireAuth, async (req, res) => {
  if (blastStatus.running) return res.status(400).json({ error: 'Blast already running' });

  const { unit, type, message, personalized, mediaPath } = req.body;
  ...
  let query = supabase.from('contacts').select('*').eq('user_id', req.user.id).eq('unit', unit);
  ...
  blastStatus = {
    ...
    total: contacts.length, log: [], unit, type: type || 'all', waKey
  };
  ...
  (async () => {
    ...
    for (const contact of contacts) {
      ...
      if (!phone) { blastStatus.skipped++; continue; }
